"""Hospital DB + PhilHealth Annex A/B rule engine."""
...
class RuleEngine:
    ...
    def _check_hospital_db(
        ...
        try:
            if hospital_name:
                rows = conn.execute(
                    "SELECT service_Name, service_Price, service_Origin "
        ...
        best_score = 0
        best_row = None
        for name, price, origin in rows:
            score = fuzz.token_sort_ratio(norm, _normalize_text(name))
            if score > best_score:
                best_score = score
                best_row = (name, price, origin)
